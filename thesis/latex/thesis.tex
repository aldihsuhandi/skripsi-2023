          ...
          \begin{enumerate}
              ...
              \item \textit{Front end}
                    ...
                    \begin{itemize}
                        ...
                        \textit{Virtual DOM} (\textbf{VDOM}) disebut juga sebagai \textit{React} \textit{DOM} adalah konsep yang dibuat untuk memudahkan \textit{developer} dengan menghilangkan kebutuhan untuk manipulasi \textit{DOM} secara manual. \textit{VDOM} adalah sebuah representasi UI dari browser yang disinkronkan dengan \textit{DOM} yang asli, \textit{developer} ketika melakukan perubahan UI dalam \textit{React} pertama yang dilakukan adalah merubah \textit{VDOM} lalu dilakukan perubahan di \textit{DOM} yang asli \autocite{React_VDOM_DOM}. Dengan \textit{Virtual DOM} \textit{React} dapat melakukan perubahan dengan efisien dengan melakukan komparasi element yang baru dengan yang sebelumnya dan hanya melakukan \textit{update} di komponent yang ada perubahan \autocite{React_rendering_needed}.
                        ...
                        Deklaratif dimana di \textit{React} untuk melakukan perubahan pada UI, hal yang perlu dilakukan adalah memberi bagaimana keadaan dan tampilan UI yang baru dan \textit{React} akan melakukan \textit{update} untuk menampilkan UI yang baru, pengguna tidak perlu secara \textit{Imperative} menangani transisi dari keadaan UI yang lama ke yang baru \autocite{React_official}.
                        ...
                        \textit{React} \textit{Hooks} sendiri adalah fitur yang disediakan supaya dapat menggunakan fitur lain dari \textit{React} dalam komponen. \textit{Hooks} dapat memiliki macam dan dapat di gabungkan berbagai tipe untuk membuat Hook baru \autocite{React_general_hooks} \textit{Hooks} yang dipakai dalam proyek ini adalah \textit{Hook useEffect} dan useState.

                        \item \textit{useEffect}

                        \textit{Hook useEffect} digunakkan untuk melakukan sinkronisasi dengan sistem eksternal, \textit{useEffect} ini akan dijalankan dalam dua situasi, pertama ketika sebuah komponen selesai \textit{render} dan kedua setiap kali ada perubahan \textit{dependency} \textit{useEffect} tersebut. Contoh yang diberikan oleh dokumentasi di web resmi \textit{React} adalah membuat koneksi server, melakukan \textit{logging} ketika sebuah komponen muncul di layar. Dalam proyek ini contoh penggunaan \textit{Hook} \textit{useEffect} adalah untuk melakukan pengambilan data dari \textit{database} setelah sebuah page atau komponen sudah selesai ditampilkan atau di \textit{render} oleh \textit{React} \autocite{React_useEffect}.
...
useEffect(() => {
    console.log("mounted");
                        ...
                        \textit{Hook useState} tergolong dalam \textit{State} \textit{Hooks} yang dideskripsikan sebagai fitur yang memberi fungsionalitas kepada komponen sehingga dapat mengingat informasi seperti input user \autocite{Banks_Porcello_2020}. \textit{Hook useState} memiliki dua fungsionalitas yang penting, yaitu menyimpan data untuk komponent yang akan di\textit{render} dan menjadi \textit{trigger} untuk me-\textit{render} ulang jika terjadi perubahan dalam data tersebut. Dalam \textit{React} ketika ada perubahan maka komponen yang bersangkutan di \textit{VDOM} akan diubah dan perubahan itu direfleksikan di \textit{DOM} yang sesungguhnya dengan melakukan \textit{rerender}, dengan menggunakkan \textit{useState} maka data sebelum terjadinya \textit{render} lagi akan tersimpan atau “diingat”, dan dengan \textit{useState} data tersebut dapat diubah sesuai dengan kebutuhan sehingga melakukan perubahan \textit{VDOM} dan DOM, \textit{rerender} dilaksanakan \autocite{React_useState}.

                        Cara deklarasi \textit{Hook useState} adalah sebagai berikut:
...
const [name, setName] = useState("initialValues");
                        ...
                        \textit{Bundling} adalah proses dimana dilakukan penggabungan \textit{file} untuk menjadi beberapa \textit{bundles} yang teroptimisasi untuk digunakkan oleh \textit{browser} \autocite{Nextjs_bundling}.
