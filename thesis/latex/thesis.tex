          ...
          \begin{enumerate}
              ...
              \item \textit{Front end}
                    ...
                    \begin{itemize}
                        ...
                        State adalah variabel yang dikelola oleh komponen yang membuat dan menggunakannya sendiri, dan \textit{Props} adalah variabel adalah data/informasi yang diberikan kepada komponen dari luar mirip seperti parameter yang diberikan sebuah \textit{Function} \autocite{React_state_props}.
                        ...
                        Hook \textit{useEffect} digunakkan untuk melakukan sinkronisasi dengan sistem eksternal, \textit{useEffect} ini berjalan setelah dalam 2 situasi, pertama ketika sebuah komponen selesai \textit{render} dan kedua setiap kali dibutuhkan dijalankan lagi yang dimana dapat diatur di dependency dari sebuah \textit{useEffect}. Contoh yang diberikan oleh dokumentasi di web resmi \textit{React} adalah membuat koneksi server, melakukan logging ketika sebuah komponen muncul di layar. Dan dalam proyek ini contoh penggunaan Hook \textit{useEffect} adalah untuk melakukan pengambilan data dari database setelah sebuah page atau komponen sudah selesai ditampilkan atau di \textit{render} oleh \textit{React} \autocite{React_useEffect}.
...
useEffect(() => {
    console.log("mounted");
                        ...
                        Dan dari kutipan kode diatas adalah penggunaan \textit{useEffect} yang mirip dengan yang sebelumnya tetapi dengan mengunakkan fitur \textit{dependencies} yang ada, dimana \textit{useEffect} akan berjalan ketika komponen yang memanggil selesai melakukan \textit{render} dan juga berjalan lagi ketika terjadi \textit{re-render} dan nilai dari variabel “count” terjadi perubahan di \textit{re-render} yang terjadi.
...
const [name, setName] = useState("initialValues");
                        ...
                        \textit{Hook} \textit{useState} dipanggil seperti \textit{Function} yang menerima parameter sebuah nilai awalan (di kutipan kode bernilai “initialValues”) dan mengembalikan/\textit{return} array dengan 2 value di elemen pertama \textit{array} adalah value untuk mengakses data (di kutipan kode bernama “name”) dan elemen kedua adalah sebuah \textit{Function} cara melakukan perubahan dari value (di kutipan kode bernama “setName”). 

                        Data yang disimpan oleh “name” pada awalnya akan beruba “initialValues” sesuai dengan nilai yang diberikan pada \textit{useState} dan akan berubah ketika “setName” dipanggil dan diberikan nilai baru, dimana “name” akan mengikuti nilai yang baru \autocite{React_useState}.
                        ...
                        \textit{Nextjs} sudah dikonfigurasikan untuk menangani proses translasi kode menjadi versi kode yang dapat dibaca oleh browser \autocite{Nextjs_compiling}. Dari kecepatan kompilasi sendiri, \textit{compiler} yang default oleh \textit{NextJs} itu 17 kali lebih cepat dari \textit{Babel} (\textit{compiler} lain yang biasa digunakkan di \textit{Create-React-App}), dan sudah menjadi \textit{compiler} bawaan/\textit{default} ketika menggunakkan \textit{NextJs} diatas versi 12 \autocite{Nextjs_depth_compiler}.

                        \item \textit{code transformation} \textit{Minifying}
                        
                        \textit{Minifying} adalah proses dimana terjadi proses penghapusan \textit{formatting}, \textit{comments} dari kode, sehingga hasil akhir ukuran file menjadi lebih kecil \autocite{Nextjs_minifiying}.

                        \item \textit{code transformation} \textit{Bundling} 
                        
                        \textit{Bundling} adalah proses dimana dilakukan penggabungan \textit{Merging} file-file dan yang ada menjadi bundles yang teroptimisasi untuk digunakkan oleh \textit{browser} \autocite{Nextjs_bundling}.
                        ...
                        \textit{react-paginate} adalah \textit{Library} untuk fitur paginasi konten atau data yang tidak muat di satu halaman. Cara menggunakannya adalah dengan spesifikasi styling untuk komponen paginasi, halaman pertama, apa yang terjadi jika di klik, dan jangkauan dari halaman yang ditampilkan \autocite{react-paginate}.
