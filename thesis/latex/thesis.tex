...
\includepdf[pages=1]{pdf/Halaman_Originalitas.pdf}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        Berikut adalah isi dari masing-masing pertanyaan yang ditanyakan kepada para \textbf{pembeli}, \textit{"Apakah anda sebagai pengguna yang bertujuan untuk membeli barang, dapat menggunakkan fitur filter Interest Level kami untuk mencari barang yang tepat?"} dan \textit{"Apakah anda tertarik atas rekomendasi yang berada di Home Page kami?"}. Kedua pertanyaan tersebut secara langsung menanyakan tentang kepuasan para pembeli setelah menggunakan kedua fitur utama yang aplikasi ini berikan, yaitu fitur penyaringan tingkat minat penggguna (\textit{interest level filtering}) dan sistem rekomendasinya. Hasil yang didapat untuk pertanyaan pertama adalah sebanyak 57.9\% memberi nilai 'lima' dan 42.1\% untuk nilai 'empat'. Sedangkan untuk pertanyaan kedua, hasil yang didapat adalah sebanyak 63.2\% dan 36.8\% yang masing-masing memberi nilai 'lima' dan 'empat'. Dengan demikian, dapat disimpulkan bahwa semua pembeli yang telah mencoba aplikasi \textit{ShumiShumi} merasa \textbf{puas} dengan fitur yang diberikan dan desain yang ditampilkan. 

        \begin{figure}[h]
            \centering
            \includegraphics[scale=0.65]{images/evaluasi ui user/Survei 2 - Pertanyaan 1.png}
            \caption{Bukti survei untuk tingkat kepuasan para pembeli mengenai fitur \textit{interest level filtering}}
        \end{figure}
        
        \begin{figure}[h]
            \centering
            \includegraphics[scale=0.65]{images/evaluasi ui user/Survei 2 - Pertanyaan 2.png}
            \caption{Bukti survei untuk tingkat kepuasan para pembeli mengenai sistem rekomendasinya}
        \end{figure}

        Sedangkan, berikut adalah isi dari masing-masing pertanyaan yang ditanyakan kepada para \textbf{penjual}, \textit{"Apakah adanya fitur interest level ini membantu anda untuk menjual produk anda ke pembeli yang tepat?"} dan \textit{"Apakah keberadaan sistem rekomendasi membantu produk yang anda jual untuk ditemukan oleh banyak calon pembeli?"}. Kedua pertanyaan tersebut secara langsung menanyakan tentang kepuasan para penjual mengenai keberadaan dan kegunaan fitur penyaringan tingkat minat penggguna (\textit{interest level filtering}) dan sistem rekomendasi terhadap tingkat penjualan toko mereka. Hasil yang didapat dari pertanyaan pertama adalah sebanyak 14.3\% memberi nilai 'lima', 42.9\% memberi nilai 'empat' dan 42.9\% memberi nilai 'tiga'. Sedangkan untuk pertanyaan kedua, sebanyak 57.1\% memberi nilai 'lima', 28.6\% memberi nilai 'empat' dan sebanyak 14.3\% memberi nilai 'tiga'. Dari variasi penilaian tersebut dapat disimpulkan bahwa para penjual merasa \textbf{cukup puas} dengan adanya fitur yang diberikan dan desain yang ditampilkan. 
        \newpage
        \begin{figure}[h]
            \centering
            \includegraphics[scale=0.65]{images/evaluasi ui merchant/Survei 3 - Pertanyaan 1.png}
            \caption{Bukti survei untuk tingkat kepuasan para penjual mengenai mengenai fitur \textit{interest level filtering}}
        \end{figure}

        \begin{figure}[h]
            \centering
            \includegraphics[scale=0.65]{images/evaluasi ui merchant/Survei 3 - Pertanyaan 2.png}
...
\hline
\textit{Testcase} & \textit{Scenario} & \textit{Step} & \textit{Expected outcome} \\ \hline
\endfirsthead
\endhead
\multirow{4}{*}{\textit{Register page}} & \textit{User can register} & \textit{\begin{tabular}[c]{@{}l@{}}- click register button\\ - fill out register form\\ - click register\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}Redirect \\ To activate page\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}Register failed when\\ date of birth\\ is invalid\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- click register button\\ - fill out form with invalid\\   date of birth\\ - click register\end{tabular}} & \textit{Show error message} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}Register failed when identifier \\ Is already taken\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- click register button\\ - fill out form with invalid\\   email, phonenumber \\   or username\\ - click register\end{tabular}} & \textit{Show error message} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}Register failed when \\ password doesn’t \\ Meets the minimum criteria\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- click register button\\ - fill out form invalid password\\ - click register\end{tabular}} & \textit{Get error message} \\ \hline
\multirow{4}{*}{\textit{Login page}} & \textit{User can login} & \textit{\begin{tabular}[c]{@{}l@{}}- click login button\\ - fill out login form\\ - click login\end{tabular}} & \textit{Redirect to homepage} \\ \cline{2-4} 
 & \textit{User not exist} & \textit{\begin{tabular}[c]{@{}l@{}}- click login button\\ - fill out with invalid user email\\ - click login\end{tabular}} & \textit{Get error message} \\ \cline{2-4} 
 & \textit{User is not active} & \textit{\begin{tabular}[c]{@{}l@{}}- click login button\\ - fill out with inactive user\\   email\\ - click login\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}Redirect to\\ activate page\end{tabular}} \\ \cline{2-4} 
 & \textit{Wrong password} & \textit{\begin{tabular}[c]{@{}l@{}}- click login\\ - fill out form with \\   invalid password\\ - click login\end{tabular}} & \textit{Get error message} \\ \hline
\multirow{3}{*}{\textit{Activate page}} & \textit{User can be activated} & \textit{\begin{tabular}[c]{@{}l@{}}- fillout the correct otp\\ - click activate\end{tabular}} & \textit{Redirect to login page} \\ \cline{2-4} 
 & \textit{Invalid otp} & \textit{\begin{tabular}[c]{@{}l@{}}- fillout form with invalid otp\\ - click activate\end{tabular}} & \textit{Get error message} \\ \cline{2-4} 
 & \textit{Resend otp} & \textit{- click resend otp} & \textit{Got new OTP Email} \\ \hline
\multirow{2}{*}{\textit{Home page}} & \textit{\begin{tabular}[c]{@{}l@{}}Check navbar \\ when not logged in\end{tabular}} & \textit{homepage when not logged in} & \textit{\begin{tabular}[c]{@{}l@{}}Register and login\\ button appear in\\ navbar\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}Check navbar \\ when logged in\end{tabular}} & \textit{homepage when logged in} & \textit{\begin{tabular}[c]{@{}l@{}}Profile appear in \\ Navbar\end{tabular}} \\ \hline
\multirow{2}{*}{\textit{Search page}} & \textit{User can search item} & \textit{\begin{tabular}[c]{@{}l@{}}- fillout search bar\\ - adjust filter \\ - click filter\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}Show item with the\\ correct criteria\end{tabular}} \\ \cline{2-4} 
 & \textit{Searching not found any item} & \textit{\begin{tabular}[c]{@{}l@{}}- search invalid itemname\\ - click search\end{tabular}} & \textit{show not found widget} \\ \hline
\multirow{9}{*}{\textit{\begin{tabular}[c]{@{}l@{}}Item detail\\ Page\end{tabular}}} & \textit{\begin{tabular}[c]{@{}l@{}}User can open \\ item detail page\end{tabular}} & \textit{- click item widget} & \textit{show item detail page} \\ \cline{2-4} 
 & \textit{User can wishlist item} & \textit{- click wishlist button} & \textit{\begin{tabular}[c]{@{}l@{}}wishlist button will \\ change to show item\\ is in wishlist\end{tabular}} \\ \cline{2-4} 
 & \textit{User can unwishlist item} & \textit{- click wishlist button} & \textit{\begin{tabular}[c]{@{}l@{}}wishlist button will \\ change to show\\ item is not in wishlist\end{tabular}} \\ \cline{2-4} 
 & \textit{User can add item to cart} & \textit{\begin{tabular}[c]{@{}l@{}}- change cart quantity\\ - click add to cart button\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}item will be in cart with\\ appropriate quantity\end{tabular}} \\ \cline{2-4} 
 & \textit{User can create discussion} & \textit{\begin{tabular}[c]{@{}l@{}}- fillout discussion form\\ - click post\end{tabular}} & \textit{show discussion} \\ \cline{2-4} 
 & \textit{User can edit discussion} & \textit{\begin{tabular}[c]{@{}l@{}}- click edit discussion\\ - fill out form\\ - click submit\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}show edited \\ discussion\end{tabular}} \\ \cline{2-4} 
 & \textit{User can delete discussion} & \textit{- click delete discussion} & \textit{\begin{tabular}[c]{@{}l@{}}remove discussion \\ from page\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}Merchant can update \\ item information\end{tabular}} & \textit{- click update button} & \textit{\begin{tabular}[c]{@{}l@{}}will be redirect to\\ update item form\end{tabular}} \\ \cline{2-4} 
 & \textit{Merchant can delete item} & \textit{- click delete button} & \textit{\begin{tabular}[c]{@{}l@{}}will be redirect to \\ homepage and item is\\ removed\end{tabular}} \\ \hline
\multirow{2}{*}{\textit{Wishlist page}} & \textit{user can search wishlist} & \textit{\begin{tabular}[c]{@{}l@{}}- fillout search bar\\ - adjust filter \\ - click filter\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}Show wishlisted\\ item with the\\ correct criteria\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}user can remove item \\ from wishlist\end{tabular}} & \textit{- click remove item} & \textit{\begin{tabular}[c]{@{}l@{}}remove item from \\ wishlist\end{tabular}} \\ \hline
\multirow{6}{*}{\textit{Cart page}} & \textit{user can select item} & \textit{\begin{tabular}[c]{@{}l@{}}- click checkmark field when\\   unselected\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}item will be selected\\ and total cart price\\ will be updated\end{tabular}} \\ \cline{2-4} 
 & \textit{user can unselect item} & \textit{\begin{tabular}[c]{@{}l@{}}- click checkmark field when\\   selected\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}item will be unselected\\ and total cart price\\ will be updated\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}user can adjust \\ selected item quantity\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- select item in cart\\ - update item quantity\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}item quantity\\ will change and cart\\ total price will be\\ updated\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}user can adjust \\ unselected item quantity\end{tabular}} & \textit{- update item quantity} & \textit{\begin{tabular}[c]{@{}l@{}}item quantity \\ will change\end{tabular}} \\ \cline{2-4} 
 & \textit{user can buy selected item} & \textit{\begin{tabular}[c]{@{}l@{}}- select some item\\ - click buy\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}will be redirected to\\ transaction payment\\ page\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}click buy item \\ when there are \\ no selected item\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- unselect all item\\ - click buy\end{tabular}} & \textit{Show error message} \\ \hline
\textit{Payment page} & \textit{\begin{tabular}[c]{@{}l@{}}user can pick \\ payment method\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- select payment \\   method\\ - click pay\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}will be redirected into\\ transaction detail page\end{tabular}} \\ \hline
\multirow{3}{*}{\textit{\begin{tabular}[c]{@{}l@{}}Transaction\\ page\end{tabular}}} & \textit{User can view all transaction} & \textit{- open transaction page} & \textit{show all transaction} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}User can filter transaction\\ by status\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- open transaction page\\ - change status filter\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}show transaction\\ with appropriate\\ status\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}User can view transaction\\ detail\end{tabular}} & \textit{- click see detail button} & \textit{\begin{tabular}[c]{@{}l@{}}will be redirect to\\ transaction detail\\ page\end{tabular}} \\ \hline
\multirow{2}{*}{\textit{Review page}} & \textit{user can create review} & \textit{\begin{tabular}[c]{@{}l@{}}- open review page\\ - click see \\   ‘waiting to be reviewed’\\ - click review\\ - fill out review form\\ - click submit review\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}will be redirected to\\ past review list page\end{tabular}} \\ \cline{2-4} 
 & \textit{user can see past review} & \textit{\begin{tabular}[c]{@{}l@{}}- open review page\\ - click ‘see reviews’\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}will be redirected to\\ past review list page\end{tabular}} \\ \hline
\multirow{3}{*}{\textit{Merchant page}} & \textit{\begin{tabular}[c]{@{}l@{}}user can see \\ merchant reviews\end{tabular}} & \textit{- click 'merchant review'} & \textit{\begin{tabular}[c]{@{}l@{}}will be redirected to\\ merchant review page\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}user can search \\ merchant items\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- fillout search bar\\ - click search\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}Show item with the\\ correct criteria\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}merchant can \\ create new item\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- click create item \\ - fill out create item form\\ - click submit\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}will be redirected to\\ item detail page\end{tabular}} \\ \hline
\multirow{3}{*}{\textit{profile page}} & \textit{\begin{tabular}[c]{@{}l@{}}user can update\\ user information\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- click update profile\\ - fill out update form\\ - click submit\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}redirect to profile page\\ with updated \\ information\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}user can become\\ a merchant\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- fill out location\\ - click 'become a merchant'\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}user become a\\ merchant\end{tabular}} \\ \cline{2-4} 
 & \textit{\begin{tabular}[c]{@{}l@{}}user failed to become\\ a merchant\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- dont fill out locatiaon\\ - click 'become a merchant'\end{tabular}} & \textit{Show error message} \\ \hline
\textit{Logout} & \textit{user can logout} & \textit{- click logout button} & \textit{\begin{tabular}[c]{@{}l@{}}redirect to homepage\\ without any login\\ information\end{tabular}} \\ \hline
\textit{Recommend} & \textit{\begin{tabular}[c]{@{}l@{}}user can get \\ item recommendation\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}- login\\ - open item detial\\ - go to homepage\end{tabular}} & \textit{\begin{tabular}[c]{@{}l@{}}user should be \\ getting \\ recommendation\\ base on their\\ activity\end{tabular}} \\ \hline
...
\begin{enumerate}
    \item \textit{Shumishumi} membantu pengguna untuk mencari produk yang lebih cocok dengan cara mencari produk dengan menggunakan filtrasi \textit{interest level} untuk mendapatkan produk yang lebih spesifik dibanding dengan seberapa dalam mereka dihobi tertentu dan fitur sistem rekomendasi dari website \textit{shumishumi} membantu pengguna untuk menemukan barang dengan kriteria sama dengan apa yang sedang pengguna minati.
    \item \textit{Shumishumi} membantu penjual untuk memasarkan produk mereka ke pengguna yang lebih tepat dengan cara memberikan kriteria \textit{interest level} yang tepat dan fitur rekomendasi di \textit{website shumishumi} membantu penjual dengan cara membuat produk penjual dapat dilihat oleh pengguna-pengguna yang sedang meminati kriteria yang sama dengan produk penjual.
\end{enumerate}


\subbab{Saran}
Berdasarkan dari pengkajian hasil penelitian dan pembuatan proyek, ada bebrapa saran yang dapat diambil bagi seseorang yang ingin membuat sebuah \textit{marketplace} ataupun peneliti selanjutnya yang ingin meneruskan penelitian disubyek ini, seperti:

\begin{enumerate}
    \item Bagi seseorang yang ingin memebuat sebuah \textit{marketplace}

    Produk \textit{filtering} dengan kriteria tambahan dalam kasus ini itu adalah \textit{interest level} atau level kemahiran pengguna dalam suatu hobi dan sistem rekomendasi adalah fitur yang harus dimiliki oleh sebuah \textit{marketplace}, dikarenakan dengan kriteria tambahan ini pengguna akan semakin cepat menemukan barang yang sesuai dengan kriteria mereka dan membelinya. Sedangkan fitur rekomendasi akan membantu pengguna untuk menemukan barang yang mereka cari dengan waktu yang singkat. 

    \item Bagi peneliti selanjutnya
    
    Adapun beberapa saran yang perlu diketahui oleh peneliti selanjutnya, yaitu:
    \begin{enumerate}
        \item Peneliti selanjutnya diharapkan untuk dapat menggunakan algoritma yang lebih kompleks untuk desain sistem rekomendasinya, atau menambahkan kriteria ke \textit{knowledge graph} yang akan membuat sistem rekomendasi ini lebih akurat.

        \item Peneliti selanjutnya juga diharapkan untuk menambahkan fitur-fitur lain, agar meningkatkan kepuasan pengguna dan penjual dalam melakukan aktivitas jual beli, fitur yang bisa ditambahkan adalah:
        \begin{itemize}
            ...
            \item Mengubah desain sistem menjadi \textit{microservice} dibanding \textit{monolith}, untuk mengingkatkan performa dan menyederhanakan \textit{codebase}.
        \end{itemize}
    \end{enumerate}
\end{enumerate}
