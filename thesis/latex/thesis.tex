...
\geometry{a4paper, top=2.5cm, bottom=2.5cm,
          left=4.0cm, right=2.5cm, includehead, includefoot}
...
\begin{document}
\newgeometry{a4paper, top=2.5cm, bottom=2.5cm,
          left=4.0cm, right=2.5cm, includehead, includefoot, asymmetric}
\setstretch{1.5}
\pagenumbering{roman}
\date{}
...
\includepdf[pages=1]{pdf/title-fixed.pdf}

\newpage
\addcontentsline{toc}{section}{\protect\numberline{Halaman Persetujuan Dosen Pembimbing}}
\includepdf[pages=1]{pdf/ttd_basah.pdf}

\newpage
\addcontentsline{toc}{section}{\protect\numberline{Halaman Pernyataan Persetujuan Publikasi Skripsi}}
\includepdf[pages=1]{pdf/pernyataan_aldih.pdf}
\newpage
\includepdf[pages=1]{pdf/pernyataan_chandra.pdf}
\newpage
\includepdf[pages=1]{pdf/pernyataan_seto.pdf}

\newpage
\addcontentsline{toc}{section}{\protect\numberline{}Abstrak}
\includepdf[pages=1]{pdf/Abstrak-fix.pdf}
...
Ruang lingkup dari proyek ini adalah perancangan sebuah aplikasi \textit{marketplace} terfokus untuk hobi dengan fitur utama sebuah filter terhadap \textit{interest level} sebuah hobi, untuk menghindari pembahasan dan implementasi yang meluas dan supaya lebih terfokus maka ditetapkan batasan-batasan atau ruang lingkup sebagai berikut:
\begin{enumerate}
    \item Aplikasi hanya berbasis dan berada dalam bentuk aplikasi \textit{website}.
    \item Fitur utama untuk aplikasi ini adalah filter \textit{interest level} dan sistem rekomendasi sebagai fitur pendukung.
...
Bab kelima ini akan memaparkan beberapa kesimpulan yang dapat diambil dan saran - saran yang didasarkan pada hasil yang didapatkan, di bab ini juga akan menyimpulkan apakahh ide aplikasi ini mempunyai potensi tinggi jika diimplementasikan didunia nyata.
...
Berdasarkan teori diatas, jenis penilaian \textit{interest level} yang digunakan di dalam aplikasi ShumiShumi ini diklasifikasikan menjadi tiga macam, yaitu \textit{Beginner, Intermediate} dan \textit{Enthusiast}. Masing-masing menyerupai tingkat penilain minat yang sudah ada. Disini, \textit{Beginner} (pemula/kasual) setara dengan penghobi yang masih baru atau \textit{casual} atau yang memiliki interest level rendah, sedangkan \textit{Intermediate} (kelas menengah) ditujukan pada orang-orang yang sudah lebih serius menjalankan hobinya atau yang memiliki interest level sedang dan \textit{Enthusiast} (penggemar berat) diperuntukkan untuk orang-orang yang sudah sangat serius mendalami suatu hobi atau yang memiliki interest level tinggi. Tujuannya adanya penilaian ini adalah untuk mempermudah mengelompokkan para pengguna berdasarkan minat hobi mereka masing-masing.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                \begin{figure}[h]
                    \centering
                    \includegraphics[scale=0.25]{images/uml satzinger/domain class diagram/general-special relationships.png}
                    \caption{Contoh dari hubungan \textit{Generalization/Specialization}}
                    \label{fig:generalspecial}
                \end{figure}

                Pada gambar \ref{fig:generalspecial}, dapat dilihat bahwa \textit{Car, Bus} dan \textit{Truck} merupakan tipe-tipe spesial dari \textit{MotorVehicle}, sedangkan \textit{SportsCar, Sedan} dan \textit{BigCar} adalah tipe-tipe spesial dari \textit{Car}. 

                Di dalam relasi ini juga, \textit{subclass} yang ada dapat "mewarisi" suatu karakteristik dari \textit{superclass}-nya. Hal ini dinamakan sebagai \textit{Inheritance}. \textit{Inheritance} memungkinkan \textit{subclasses} untuk berbagi karakteristik \textit{superclass} mereka. Di dalam pendekatan \textit{object oriented}, \textit{inheritance} merupakan sebuah konsep kunci yang dimungkinkan dikarenakan adanya hierarki \textit{generalization/specialization}. Seperti contoh yang ada pada \textit{figure 2.13}, dapat dilihat bahwa \textit{Sedan} adalah jenis \textit{Car} yang khusus dan \textit{Car} adalah jenis \textit{MotorVehicle} yang khusus. Istilah lain dari relasi ini adalah \textit{"Is A" Relationship}\autocite[103-114]{uml-satzinger}.

                \item \textit{Whole-Part Relationships}
                ...
                \begin{figure}[h]
                    \centering
                    \includegraphics[scale=0.5]{images/uml satzinger/domain class diagram/composition.png}
                    \caption{Bentuk dari simbol \textit{composition}}
                \end{figure}
        ...
        \item \textit{Design Class Diagram}
        
        \textit{Design Class Diagram} berisikan definisi akhir dari setiap \textit{class} yang ada dalam sebuah sistem \textit{software} yang bersifat \textit{object-oriented}. \textit{Domain Model Class Diagram} menunjukkan sekumpulan masalah \textit{domain classes} dan asosiasinya. Sedangkan, \textit{design class diagram} menampilkan hal-hal yang lebih detail dari itu, seperti atribut yang dideklarasi secara \textit{public} atau \textit{private}, tipe-tipe dari atributnya hingga mendefinisikan metode dan parameter yang digunakan pada \textit{class} tersebut. Singkatnya, \textit{design class diagram} adalah bentuk lebih rinci dan teknikal dari \textit{domain model class diagram}\autocite[376-382]{uml-satzinger}.

        \begin{itemize}
            \item \textit{Design Class Stereotypes}
            
            \textit{Stereotype} pada \textit{design class} merupakan sebuah cara sederhana untuk mengkategorikan elemen model sebagai tipe-tipe tertentu. \textit{Stereotype} memperluas definisi dasar elemen model dengan mengindikasikan bahwa elemen tersebut memiliki karakteristik khusus yang ingin ditonjolkan\autocite[376-382]{uml-satzinger}.

            Terdapat empat jenis \textit{design class} yang dapat dibilang sebagai \textit{stereotypes} yang standar, antara lain \textit{entity class, boundary} atau \textit{view class, controller class} dan \textit{data access class}\autocite[376-382]{uml-satzinger}.
        ...
        Berikut adalah salah satu contoh sederhana dari bentuk \textit{design class diagram} bersama dengan relasi dan \textit{multiplicity}-nya. 
        \newpage
        \begin{figure}[h]
            \centering
            \includegraphics[scale=0.5]{images/uml satzinger/design class diagram/contoh design class diagram.png}
            \caption{Contoh sederhana dari \textit{design class diagram}}
        \end{figure}
    ...
    \textit{Activity Diagram} merupakan sebuah diagram UML yang digunakan untuk menggambarkan berbagai aktivitas pengguna (atau sistem), seseorang atau komponen yang menyelesaikan setiap aktivitas dan aliran berurutan dari aktivitas-aktivitas tersebut\autocite[60-63]{uml-satzinger}. Berikut adalah rangkuman dari simbol-simbol yang digunakan dalam \textit{activity diagram}:
    ...
    Dapat dilihat pada gambar \ref{fig:activitynotation} diatas, \textit{activity} masing-masing digambarkan dengan sebuah oval pipih. Panah penghubung menunjukkan urutan dari \textit{activity}. Lingkaran hitam menggambarkan titik mulai dari \textit{activity diagram} dan lingkaran hitam di dalam lingkaran putih menggambarkan akhir dari \textit{activity diagram} tersebut. Simbol berlian menggambarkan \textit{decision point} yang dimana aliran proses akan mengikuti satu jalur atau jalur yang lain. Garis horizontal yang tebal merupakan sebuah \textit{synchronization bar} yang membagi beberapa jalur bersamaan atau menggabungkan kembali jalur yang jalan bersamaan tersebut. \textit{Swimlane} mewakili sebuah agen yang melakukan aktivitas. Disebut \textit{swimlane} karena setiap agen mengikuti jalur yang sejajar dengan agen lain dalam alur kerja, seperti halnya perenang di kolam renang\autocite[60-63]{uml-satzinger}.
    \newpage
    \begin{figure}[h]
        \centering
        \includegraphics[scale=0.35]{images/uml satzinger/activity diagram/contoh sederhana activity diagram.png}
        \caption{Contoh sederhana dari \textit{activity diagram} dalam kasus pembelian suatu tiket}
    \end{figure}
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[scale=0.5]{images/uml satzinger/use case diagram/contoh use case diagram dgn includes.png}
        \caption{Contoh sederhana \textit{use case diagram} dengan relasi \textit{includes}-nya}
        \label{fig:contohusecase}
    \end{figure}

    Dari gambar \ref{fig:contohusecase} diatas dapat dilihat bahwa \textit{fill shopping cart} juga memiliki \textit{search for item, view product comments and ratings} dan \textit{view accessory combinations}. Dengan demikian, pengguna dapat melihat komentar sekaligus menjalankan \textit{use case 'fill shopping cart'}. Hubungan-hubungan antara \textit{use case} seperti ini digambarkan dengan garis putus-putus dengan panah penghubung yang menunjuk ke arah \textit{use case} yang dimaksud. Hubungan ini dapat dibaca sebagai "\textit{fill shopping cart} termasuk (\textit{includes}) \textit{search for item}". Ini yang dinamakan dengan \textit{include relationship} atau \textit{uses relationship}\autocite[81-87]{uml-satzinger}.
    
    \item \textit{Sequence Diagram}
    
    \textit{Sequence Diagram} atau \textit{System Sequence Diagram} (SSD) digunakan untuk menggambarkan aliran informasi ke dalam dan ke luar dari bagian sistem yang terotomatisasi. Dengan demikian, \textit{sequence diagram} mendokumentasikan \textit{input} dan \textit{output} serta mengidentifikasi interaksi antara \textit{actor} dengan sistem. Diagram ini adalah alat yang efektif untuk membantu dalam desain awal dari \textit{user interface} dengan mengidentifikasi informasi spesifik yang mengalir dari pengguna ke dalam sistem dan informasi yang mengalis ke luar dari sistem kembali ke pengguna\autocite[139-146]{uml-satzinger}.
...
\begin{figure}[h]
    \centering
    \includegraphics*[width=7cm]{./images/new kerangka berfikir.png}
...
\begin{enumerate}
    ...
    Tahapan \textit{backlog grooming} atau \textit{Backlog refinement} dilakukan untuk memastikan semua \textit{stories} yang ada di dalam \textit{backlog} sesuai dengan \textit{requirement} yang didapat dan dalam tahapan ini juga \textit{user stories} akan dipecah menjadi \textit{task-task} yang lebih kecil dan diberi \textit{story points}(satu \textit{story point} sama dengan tiga jam) berdasarkan \textit{effort} yang dibutuhkan untuk mengerjakan \textit{task} tersebut.
...
\end{longtable}
    ...
    Pada tahapan ini tim \textit{development} akan mengerjakan sprint backlog seperti yang telah direncanakan di fase sebelumnya, dalam tahapan ini juga dilaksanakan \textit{daily scrum} atau \textit{meeting} yang dilaksanakan untuk mengetahui apakah ada \textit{blocker} atau masalah yangterjadi saat masa \textit{development}.
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            Sedangkan, aplikasi Shumishumi merupakan aplikasi berbasis web, yang berarti perangkat apapun bisa menjalankan aplikasi ini, selama perangkat-perangkat tersebut mempunyai akses untuk internet dan memiliki \textit{web browser}.
    
            \item Kegunaan dan Ruang Lingkup Aplikasi
    
            Kegunaan dari aplikasi Shumishumi adalah untuk menjual belikan berbagai macam produk yang berhubungan dengan hobi. Sedangkan aplikasi agrikultur ini, sesuai dengan namanya, yakni menjual berbagai macam barang-barang kebutuhan agrikultur seperti padi, jangung, kentang, dan produk-produk sejenis lainnya\autocite{agriculture-marketplace}.
            ...
            \begin{itemize}
                \item Adanya fitur rekomendasi yang hanya dimiliki oleh aplikasi Shumishumi yang tidak dimiliki oleh aplikasi jual-beli agrikultur tersebut
        ...
        Berikut adalah perbedaan-perbedaan yang dimiliki antara aplikasi e-Nelayan dengan aplikasi Shumishumi:
        ...
        \begin{itemize}
            ...
            Sedangkan aplikasi Shumishumi merupakan sebuah aplikasi berbasis web. Yang artinya, dapat digunakan pada perangkat apa saja denagn jenis sistem operasi apa saja selama perangkat-perangkat tersebut memiliki konekasi internet dan \textit{web browser}.
            ...
            Sedangkan aplikasi Shumishumi dipergunakan untuk membantu proses jual-beli barang-barang kebutuhan hobi. Mulai dari hobi berolahraga, bermain \textit{game}, bermain musik, dan lain sebagainya. Untuk jangkauan wilayah aplikasi ShumiShumi sendiri, dapat digunakan untuk semua pengguna yang berada di Indonesia.
        ...
        \begin{itemize}
            ...
            Sedangkan aplikasi Shumishumi dipergunakan untuk membantu proses jual-beli barang-barang kebutuhan hobi. Mulai dari hobi berolahraga, bermain game, bermain musik, dan lain sebagainya. Untuk jangkauan wilayahnya sendiri, aplikasi Shumishumi dapat digunakan untuk semua pengguna yang berada di Indonesia.
            ...
            \begin{itemize}
                ...
                Para nelayan dan pembeli berdiskusi langsung melalui telepon ataupun aplikasi pesan teks pada \textit{handphone} masing-masing\autocite{c2c-fish-marketplace}. Sedangkan proses diskusi pada aplikasi Shumishumi dilakukan melalui fitur komentar pada masing-masing produk kebutuhan hobi yang dijual.
        ...
        \begin{itemize}
            ...
            Sedangkan, aplikasi Shumishumi adalah sebuah \textit{online marketplace} yang menjual-belikan barang-barang khusus kebutuhan hobi. Mulai dari hobi bermain bola, bermain musik, memasak dan lain-lain.
    
            Untuk ruang lingkup dari kedua aplikasi ini, aplikasi LIDI hanya diperuntukkan untuk perusahaan jasa sewa mobil dan masyarakat di wilayah Sidoarjo. Sedangkan, aplikasi Shumishumi dapat digunakan oleh semua pengguna yang ada di Indonesia.
            ...
            \begin{itemize}
                ...
                Aplikasi LIDI ini tidak memiliki fitur rekomendasi sama sekali, sedangkan aplikasi Shumishumi memiliki fitur tersebut pada halaman utamanya.
    
                \item Fitur Diskusi
    
                Proses diskusi antara konsumen dengan penyewa mobil adalah melalui telepon atau datang langsung ke tempat dimana perusahaan jasa sewa mobil tersebut berada. Sedangkan proses diskusi pada aplikasi Shumishumi dapat dilakukan penuh secara \textit{online}.
        ...
        Prototipe aplikasi BUM Desa Pakis ini memiliki beberapa kemiripan dengan aplikasi Shumishumi, yakni sama-sama merupakan aplikasi berbasis web, sama-sama memiliki sistem pembayaran yang terintegrasi dengan bank-bank lokal dan sama-sama memiliki sistem pencarian dan penyaringan produk berdasarkan kateogori dari produk tersebut. Kedua aplikasi ini juga memiliki beberapa perbedaan yang cukup signifikan. Perbedaan-perbedaan tersebut antara lain:
        ...
        \begin{itemize}
            ...
            Sedangkan aplikasi Shumishumi hanya menual belikan produk-produk kebutuhan hobi. Dimulai dari yang hobi bermain olahraga, bermain musik hingga bermain \textit{video game}.
            ...
            \begin{itemize}
                ...
                Prototipe aplikasi BUM Desa Pakis ini tidak memiliki sistem rekomendasi sama sekali seperti yang ada pada aplikasi Shumishumi\autocite{bum-mekarsari}.
    
                \item Fitur Diskusi
    
                Prototipe aplikasi BUM Desa Pakis ini juga tidak memiliki sebuah fitur untuk bersikusi secara \textit{online} seperti yang ada pada aplikasi Shumishumi\autocite{bum-mekarsari}.
    ...
    \begin{centering}
    \captionof{table}{Aplikasi-aplikasi berdasarkan penelitian terdahulu}
    \begin{longtable}{|p{3cm}|p{5cm}|p{5cm}|}
        ...
        + Memepersatukan bisnis-bisnis kecil ke dalam satu \textit{platform} online jual beli yang sama \newline
    ...
    \begin{centering}
    \captionof{table}{Tabel perbandingan fitur antara Shumishumi dengan aplikasi pada penelitian terdahulu (1)}
    ...
    \newpage
    Berikut dibawah ini adalah tabel perbandingan antara aplikasi Shumishumi dengan aplikasi \textit{marketplace} agrikultur\autocite{agriculture-marketplace} dan aplikasi C2C \textit{marketplace} produk perikanan menggunakan metode \textit{SMS Gateway}\autocite{c2c-fish-marketplace}.

    \begin{centering}
    \captionof{table}{Tabel perbandingan fitur antara Shumishumi dengan aplikasi pada penelitian terdahulu (2)}
    ...
    \begin{enumerate}
        ...
        Diantara kedua aplikasi ShumiShumi dengan KyouID, terdapat beberapa kesamaan dan perbedaan. Persamaannya adalah kedua-duanya merupakan sebuah aplikasi \textit{online hobby shop} dan perbedaannya adalah aplikasi KyouID hanya memasarkan produk-produk kebutuhan hobi budaya pop jepang saja, sedangkan aplikasi ShumiShumi menjualbelikan produk kebutuhan hobi untuk berbagai macam hobi, seperti berolahraga, bermain \textit{video game} dan bermain musik.

        Dari segi fitur yang diimplementasikan, terdapat beberapa perbedaan yang menonjol dari kedua aplikasi ini, perbedaan-perbedaan tersebut antara lain:

        \begin{itemize}
            \item Sistem Rekomendasi
            
            Kedua aplikasi ini sama-sama memiliki sebuah sistem rekomendasi. Yang membedakannya adalah bagaimana cara sistem rekomendasi tersebut bekerja. Pada aplikasi ShumiShumi, sistem rekomendasinya berjalan berdasarkan aktivitas yang dilakukan oleh penggunanya. Apabila aktivitas pengguna lebih banyak mengarah kepada barang-barang kebutuhan hobi olahraga (contoh: sepak bola), maka pada halaman utama aplikasi, pengguna tersebut akan direkomkendasikan barang-barang yang berhubungan dengan hobi tersebut.

            Sedangkan sistem rekomendasi yang digunakan oleh KyouID lebih mengarah dalam pemasaran produk-produk dari yang paling baru diluncurkan atau produk-produk yang sedang dalam proses \textit{open pre-order} dan produk-produk lama yang sedang dijual kembali.

            \item Sistem Diskusi
            
            Untuk fitur yang satu ini, aplikasi ShumiShumi sudah mengimplementasikannya pada masing-masing halaman produk yang sedang dijual. Sedangkan untuk aplikasi KyouID, aplikasi ini sama sekali tidak memiliki fitur diskusi atau komentar untuk produk-produk yang mereka jual.

        \end{itemize}

        \item Hobbycraft
        
        Hobbycraft adalah sebuah jaringan ritel \textit{superstore} seni dan kerajinan di Inggris yang saat ini dimiliki oleh Bridgepoint Group plc. Didirikan pertama kali oleh Haskins Group di tahun 1995 dan telah membuka lebih dari seratus toko di Inggris. Untuk aplikasi yang dikembangkan oleh Hobbycraft itu sendiri merupakan sebuah aplikasi berbasis web yang diluncurkan sekitar tahun 2010 dan 2011 yang lalu\autocite{about-hobbycraft}. Aplikasi ini menjualbelikan berbagai macam produk yang berhubungan dengan seni dan kerajinan, seperti peralatan lukis tradisional maupun digital, barang kebutuhan kerajinan kertas, peralatan bantu untuk proyek DIY (\textit{Do It Yourself}) atau kerajinan tangan dan lain sebagainya.
    ...
    \begin{centering}
        \captionof{table}{Aplikasi-aplikasi besar sejenis yang sudah berjalan lebih dulu}
        \begin{longtable}{|p{3cm}|p{5cm}|p{5cm}|}
            \hline
            Aplikasi & Kelebihan                                                                                                                 & Kekurangan \\
            \hline
        ...
        \begin{centering}
            \captionof{table}{Tabel perbandingan fitur antara ShumiShumi dengan aplikasi-aplikasi besar yang sejenis}
            \begin{longtable}{|p{3cm}|p{2cm}|p{2cm}|p{3cm}|p{2cm}|}
                \hline
...
% \begin{centering}
% \captionof{table}{Aplikasi pasaran yang serupa dengan proyek ini}
...
Berdasarkan rumusan masalah yang telah dikaji di bab pertama, pengembangan aplikasi \textit{marketplace hobby} Shumishumi ini membutuhkan berbagai macam data atau masukan dari para calon penggunanya demi menyelesaikan permasalahan yang dihadapi. Maka dari itu, penelitian ini telah mengadakan survei yang dilakukan menggunakan aplikasi \textit{Google Form}.

Survei ini bertujuan untuk membantu penelitian ini mendapatkan informasi yang dibutuhkan sebagai fondasi dasar untuk membuat dan mengembangkan aplikasi Shumishumi ini. Informasi-informasi yang ingin dicari tahu terlebih dahulu melalui survei ini antara lain sebagai berikut:
...
\begin{itemize}
    ...
    \item Mencari tahu apakah melabelkan atau menandakan produk-produk kebutuhan hobi sesuai dengan interest level atau tingkat kemampuan penggunanya dapat membantu para pengguna untuk mencari barang-barang hobi yang diinginkan
\end{itemize}
    ...
    \item Fungsi-fungsi \textit{software}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                Fitur ini dibuat untuk membantu \textit{user} jika terjadi kelupaan \textit{password} atau kata sandi akun Shumishumi
                \begin{figure}[h]
                    \centering
    ...
    \item Kebutuhan teknologi
          ...
          \begin{enumerate}
              \item \textit{Back end}
                    ...
                    Di dalam \textit{springboot} sendiri ada beberapa \textit{extension} yang dipakai, seperti:
                    \begin{enumerate}
                    ...
                    Untuk \textit{deployment} sendiri, proyek ini menggunakan \textit{Google cloud platform} (\textbf{GCP}) sebagai jasa \textit{hosting}, GCP sendiri terdiri dari aset fisik yang berada di dalam \textit{data center google} yang tersebar diseluruh dunia, diproyek ini GCP digunakan untuk meng-\textit{host} \textit{instance database} dan \textit{instance backend} dengan spesifikasi:
                    \begin{enumerate}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{table}[h]
            ...
            \multicolumn{1}{|l|}{\textit{User, merchant}: untuk membuat akun Shumishumi baru. }                                                     &                               &                               \\ \hline
...
\begin{table}[h]
    ...
    \multicolumn{1}{|l|}{\textit{Primary Actor}: \textit{User, Merchant}}             & \multicolumn{2}{l|}{\textit{Use case Type}: \textit{Essential}} \\ \hline
    \multicolumn{1}{|l|}{\textit{Stake Holders and Interest}} &                               &                               \\ \cline{1-1}
    \multicolumn{1}{|l|}{\textit{User, Merchant}: \textit{login} ke akun Shumishumi.}                                                     &                               &                               \\ \hline
...
\begin{table}[h]
    ...
    \multicolumn{1}{|l|}{\textit{Primary Actor}: \textit{User, Merchant}}             & \multicolumn{2}{l|}{\textit{Use case Type}: \textit{Essential}} \\ \hline
    \multicolumn{1}{|l|}{\textit{Stake Holders and Interest}} &                               &                               \\ \cline{1-1}
    \multicolumn{1}{|l|}{\textit{User, Merchant}: \textit{logout} dari user Shumishumi}                                                     &                               &                               \\ \hline
...
\end{table}
        ...
        \item \textit{View my merchant page}
\begin{table}[h]
    \centering
    \caption{\textit{usecase view my merchant page}}
    \begin{tabular}{|lll}
    \hline
    \multicolumn{1}{|l|}{\textit{Use Case Name}: \textit{View my merchant page}}             & \multicolumn{2}{l|}{\textit{Importance}: \textit{High}}   \\ \hline
    \multicolumn{1}{|l|}{\textit{Primary Actor}: \textit{Merchant}}             & \multicolumn{2}{l|}{\textit{Use case Type}: \textit{Essential}} \\ \hline
    \multicolumn{1}{|l|}{\textit{Stake Holders and Interest}} &                               &                               \\ \cline{1-1}
    \multicolumn{1}{|l|}{\textit{Merchant}: untuk melihat \textit{merchant page user} sekarang}                                                     &                               &                               \\ \hline
    \multicolumn{3}{|l|}{\textit{Brief Description}: \makecell[l]{\textit{entry point} untuk \textit{manage item}}}                                                                         \\ \hline
    \multicolumn{3}{|l|}{\textit{Trigger}: \textit{click button view merchant page} }                                                                                   \\ \hline
    \multicolumn{3}{|l|}{\textit{Relationship:}}                                                                              \\ \hline
    \multicolumn{3}{|l|}{}                                                                                                                     \\ \hline
    \multicolumn{3}{|l|}{\textit{Sub Flows}:}                                                                                 \\ \hline
    \multicolumn{3}{|l|}{\textit{Alternate/Exceptional Flow}:}                                                                \\ \hline
...
\end{table}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \end{figure}
            \newpage
            \item merubah jumlah barang yang ada di dalam \textit{cart}
            \begin{figure}[h]
                \centering
                \includegraphics*[height=6cm]{./diagram/activity diagram/13. cart/update cart/update cart.png}
                \caption{\textit{Activity diagram} untuk merubah jumlah barang yang ada di dalam \textit{cart}}
            \end{figure}
        ...
        \begin{figure}[h]
            \centering
            \includegraphics*[height=6cm]{./diagram/activity diagram/11-5. view my merchant page/view my merchant page.png}
            \caption{\textit{Activity diagram} untuk melihat \textit{merchant page user} sekarang}
        \end{figure}
    \end{enumerate}
    \newpage
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \newpage
            \item \textit{Sequence diagram} untuk \textit{update item information}
            \begin{figure}[h]
                \centering
                \includegraphics*[height=6cm]{diagram/sequence diagram/FE/17. item/update item/update item.png}
                \caption{\textit{Sequence diagram} untuk fitur \textit{update item information}}
            \end{figure}
        \end{itemize}
        \item \textit{Sequence diagram backend}
        \begin{itemize}
            \item \textit{Sequence diagram backend} untuk \textit{UserController.register}
            \begin{figure}[h]
                \centering
                \includegraphics*[height=6cm]{diagram/sequence diagram/BE/user controller/register/register.png}
                \caption{\textit{Sequence diagram backend} untuk \textit{UserController.register}}
            \end{figure}
            ...
            \newpage
            \item \textit{Sequence diagram backend} untuk \textit{ReviewController.query}
            \begin{figure}[h]
                \centering
                \includegraphics*[height=6cm]{diagram/sequence diagram/BE/review controller/query/detail.png}
                \caption{\textit{Sequence diagram backend} untuk \textit{ReviewController.query}}
            \end{figure}
        \end{itemize}
    \end{enumerate}
    ...
    \begin{figure}[h]
        ...
        \caption{\textit{Class diagram} untuk aplikasi Shumishumi}
    \end{figure}
    \newpage

    \item Perancangan \textit{User Interface}
    
    Berikut adalah rancangan dari \textit{User Interface} (UI) pada aplikasi Shumishumi yang telah digambarkan dalam bentuk \textit{wireframe}:
    ...
    berikut ini adalah perancangan dari \textit{database} yang dipakai oleh Shumishumi:
    \begin{figure}[h]
        \centering
    ...
    \end{figure}
    \newpage
    Berikut ini adalah kamus data untuk \textit{entity relation diagram}:
    \begin{itemize}
        \item \textit{Table item\_categories}\\
        \textit{Primary key}: category\_id \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 4 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{item\_categories}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            \endhead
            %
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & category\_id & varchar(255) & \textit{primary key} untuk \textit{table item\_categories} \\ \hline
            4 & category\_name & varchar(255) & nama kategori \\ \hline
        \end{longtable}

        \item \textit{Table hobbies}\\
        \textit{Primary key}: hobby\_id \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 4 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{hobbies}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & hobby\_id & varchar(255) & \textit{primary key} untuk \textit{table hobbies} \\ \hline
            4 & hobby\_name & varchar(255) & nama hobi \\ \hline
        \end{longtable}

        \item \textit{Table interest\_levels}\\
        \textit{Primary key}: interest\_level\_id \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 4 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{interest\_levels}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & interest\_level\_id & varchar(255) & \textit{primary key} untuk \textit{tableeinterest\_levels} \\ \hline
            4 & level\_name & varchar(255) & nama \textit{interest level} \\ \hline
        \end{longtable}

        \item \textit{Table user\_roles}\\
        \textit{Primary key}: role\_id \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 4 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{user\_roles}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & role\_id & varchar(255) & \textit{primary key} untuk \textit{table user\_roles} \\ \hline
            4 & role\_name & varchar(255) & nama \textit{user roles} \\ \hline
        \end{longtable}

        \item \textit{Table users}\\
        \textit{Primary key}: user\_id \\
        \textit{Foreign key}: \textit{role\_id} \\
        Jumlah \textit{field}: 15 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{users}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & user\_id & varchar(255) & \textit{primary key} untuk tabel \textit{users} \\ \hline
            4 & role\_id & varchar(255) & \textit{foreign key} untuk ke tabel \textit{user\_roles} \\ \hline
            5 & email & varchar(255) & \textit{field} untuk menampung \textit{email user} \\ \hline
            6 & phone\_number & varchar(255) & \textit{field} untuk menampung nomor hp \textit{user} \\ \hline
            7 & username & varchar(255) & \textit{field} untuk menampung \textit{username} \\ \hline
            8 & password & varchar(255) & \textit{field} untuk menampung \textit{password} \\ \hline
            9 & gender & varchar(255) & \textit{field} untuk menampung gender \textit{user} \\ \hline
            10 & date\_of\_birth & timestamp & \textit{field} untuk menampung tanggal lahir \textit{user} \\ \hline
            11 & location & varchar(255) & \textit{field} untuk menampung lokasi tempat tinggal \textit{user} dalam bentuk json\\ \hline
            12 & profile\_picture & varchar(255) & \textit{field} untuk menampung gambar profil \textit{user}\\ \hline
            13 & review & double & \textit{field} untuk menampung \textit{review} yang diberikan kepada \textit{user}\\ \hline
            14 & is\_active & boolean & \textit{field} yang menandakan apakah \textit{user} sudah aktif atau belum\\ \hline
            15 & is\_deleted & boolean & \textit{field} yang menandakan apakah \textit{user} sudah di\textit{delete} atau belum\\ \hline
        \end{longtable}

        \item \textit{Table items}\\
        \textit{Primary key}: item\_id \\
        \textit{Foreign key}: merchant\_id;category\_id;hobby\_id;merchant\_level\_id;user\_level\_id \\
        Jumlah \textit{field}: 15 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{items}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & item\_id & varchar(255) & \textit{primary key} untuk tabel \textit{items} \\ \hline
            4 & item\_name & varchar(255) & \textit{field} untuk menyimpan nama produk \\ \hline
            5 & item\_price & number(10) & \textit{field} untuk menyimpan harga produk \\ \hline
            6 & item\_images & varchar(255) & \textit{field} untuk menyimpan gambar-gambar produk \\ \hline
            7 & item\_description & longtext & \textit{field} untuk menyimpan deskripsi produk \\ \hline
            8 & item\_quantity & number(10) & \textit{field} untuk menyimpan quantitas produk \\ \hline
            9 & merchant\_id & varchar(255) & \textit{field} yang menjadi \textit{foreign key} ke tabel \textit{users} \\ \hline
            10 & category\_id & varchar(255) & \textit{field} yang menjadi \textit{foreign key} ke tabel \textit{item\_categories} \\ \hline
            11 & hobby\_id & varchar(255) & \textit{field} yang menjadi \textit{foreign key} ke tabel \textit{hobbies} \\ \hline
            12 & merchant\_level\_id & varchar(255) & \textit{field} yang menjadi \textit{foreign key} ke tabel \textit{interest\_levels} \\ \hline
            13 & user\_level\_id & varchar(255) & \textit{field} yang menjadi \textit{foreign key} ke tabel \textit{interest\_levels} \\ \hline
            14 & review & double & \textit{field} untuk menampung \textit{review} yang diberikan kepana \textit{item} \\ \hline
            15 & is\_deleted & boolean & \textit{field} yang menandakan apakah \textit{item} sudah di\textit{delete} atau belum\\ \hline
        \end{longtable}

        \newpage
        \item \textit{Table history\_items}\\
        \textit{Primary key}: history\_item\_id \\
        \textit{Foreign key}: item\_id;merchant\_id;category\_id;hobby\_id;merchant\_level\_id;user\_level\_id \\
        Jumlah \textit{field}: 16 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{history\_items}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & history\_items & varchar(255) & \textit{primary key} untuk tabel \textit{history\_items} \\ \hline
            4 & item\_id & varchar(255) & \textit{field} yang menjadi \textit{foreign key} ke tabel \textit{items} \\ \hline
            5 & item\_name & varchar(255) & \textit{field} untuk menyimpan nama produk \\ \hline
            6 & item\_price & number(10) & \textit{field} untuk menyimpan harga produk \\ \hline
            7 & item\_images & varchar(255) & \textit{field} untuk menyimpan gambar-gambar produk \\ \hline
            8 & item\_description & longtext & \textit{field} untuk menyimpan deskripsi produk \\ \hline
            9 & item\_quantity & number(10) & \textit{field} untuk menyimpan quantitas produk \\ \hline
            10 & merchant\_id & varchar(255) & \textit{field} yang menjadi \textit{foreign key} ke tabel \textit{users} \\ \hline
            11 & category\_id & varchar(255) & \textit{field} yang menjadi \textit{foreign key} ke tabel \textit{item\_categories} \\ \hline
            12 & hobby\_id & varchar(255) & \textit{field} yang menjadi \textit{foreign key} ke tabel \textit{hobbies} \\ \hline
            13 & merchant\_level\_id & varchar(255) & \textit{field} yang menjadi \textit{foreign key} ke tabel \textit{interest\_levels} \\ \hline
            14 & user\_level\_id & varchar(255) & \textit{field} yang menjadi \textit{foreign key} ke tabel \textit{interest\_levels} \\ \hline
            15 & review & double & \textit{field} untuk menampung \textit{review} yang diberikan kepana \textit{item} \\ \hline
            16 & is\_deleted & boolean & \textit{field} yang menandakan apakah \textit{item} sudah di\textit{delete} atau belum\\ \hline
        \end{longtable}
        
        \newpage
        \item \textit{Table sessions}\\
        \textit{Primary key}: sessions \\
        \textit{Foreign key}: user\_id \\
        Jumlah \textit{field}: 7 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{sessions}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & session\_id & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{sessions} \\ \hline
            4 & user\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{users} \\ \hline
            5 & session\_dt & timestamp & \textit{field} untuk menandakan waktu \textit{session expired} \\ \hline
            6 & is\_active & boolean & \textit{field} untuk menandakan apakah \textit{session} masih aktif atau tidak \\ \hline
            7 & is\_remembered & boolean & \textit{field} untuk menandakan apakah \textit{session} akan diingat atau tidak \\ \hline
        \end{longtable}

        \item \textit{Table cart}\\
        \textit{Primary key}: item\_id;user\_id \\
        \textit{Foreign key}: item\_id;user\_id \\
        Jumlah \textit{field}: 6 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{sessions}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & user\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{users} \\ \hline
            4 & item\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{items} \\ \hline
            5 & quantity & number(10) & \textit{field} untuk menampung kuantitas item didalam keranjang \\ \hline
            6 & selected & number(10) & \textit{field} untuk menjadi \textit{flag} apakah \textit{entry} ini sedang di-\textit{select} atau tidak \\ \hline
        \end{longtable}

        \newpage
        \item \textit{Table transactions}\\
        \textit{Primary key}: trx\_id \\
        \textit{Foreign key}: user\_id \\
        Jumlah \textit{field}: 8 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{transactions}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & trx\_id & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{transactions} \\ \hline
            4 & user\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{users} \\ \hline
            5 & payment\_type & varchar(255) & tipe pembayaran yang dipakai untuk transaksi ini \\ \hline
            6 & midtrans\_id & varchar(255) & \textit{field} untuk menyimpan \textit{payment id} midtrans \\ \hline
            7 & midtrans\_link & varchar(255) & \textit{field} untuk menyimpan link pembayaran midtrans \\ \hline
            8 & status & varchar(255) & \textit{field} untuk menyimpan status transaksi \\ \hline
        \end{longtable}

        \item \textit{Table transaction\_detail}\\
        \textit{Primary key}: trx\_detail\_id \\
        \textit{Foreign key}: trx\_id;history\_item\_id \\
        Jumlah \textit{field}: 6 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{transaction\_detail}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & trx\_detail\_id & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{transaction\_detail} \\ \hline
            4 & trx\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{transactions} \\ \hline
            5 & history\_item\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{history\_items} \\ \hline
            6 & quantity & int & \textit{field} untuk menyimpan kuantitas barang dalam transaksi \\ \hline
        \end{longtable}

        \item \textit{Table wishlist}\\
        \textit{Primary key}: item\_id;user\_id \\
        \textit{Foreign key}: item\_id;user\_id \\
        Jumlah \textit{field}: 4 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{wishlist}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
        ...
        \item \textit{Table reviews}\\
        \textit{Primary key}: review\_id \\
        \textit{Foreign key}: item\_id;user\_id;merchant\_id;interest\_level \\
        Jumlah \textit{field}: 10 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{reviews}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & review\_id & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{reviews} \\ \hline
            4 & user\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{users} \\ \hline
            5 & item\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{items} \\ \hline
            6 & merchant\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{users} \\ \hline
            7 & interest\_level & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{interest\_levels} \\ \hline
            8 & description & longtext & \textit{field} untuk menyimpan deskripsi \textit{review} \\ \hline
            9 & review\_images & varchar(255) & \textit{field} untuk menyimpan gambar-gambar \textit{review} \\ \hline
            10 & need\_review & boolean & \textit{field} untuk menjadi \textit{flag} apakah \textit{review} ini sudah dibuat atau belum \\ \hline
        \end{longtable}

        \item \textit{Table comment}\\
        \textit{Primary key}: comment\_id \\
        \textit{Foreign key}: user\_id;reply\_comment\_id;reply\_item\_id \\
        Jumlah \textit{field}: 8 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{comment}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & comment\_id & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{comments} \\ \hline
            4 & user\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{users} \\ \hline
            5 & reply\_comment\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{comments} \\ \hline
            6 & reply\_item\_id & varchar(255) & \textit{field} untuk menjadi \textit{foreign key} ke tabel \textit{items} \\ \hline
            7 & content & varchar(255) & \textit{field} untuk menampung \textit{comment} \\ \hline
            8 & is\_deleted & varchar(255) & \textit{flag} untuk menandakan apakah \textit{comment} dihapus atau tidak \\ \hline
        \end{longtable}

        \item \textit{Table clients}\\
        \textit{Primary key}: client\_id \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 5 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{clients}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & client\_id & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{comments} \\ \hline
            4 & client\_name & varchar(255) & nama dari \textit{client} \\ \hline
            5 & client\_secret & varchar(255) & \textit{secret} dari \textit{client} \\ \hline
        \end{longtable}

        \item \textit{Table otp}\\
        \textit{Primary key}: otp\_id \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 8 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{otp}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & otp\_id & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{otp} \\ \hline
            4 & otp\_dt & timestamp & sebagai waktu kapan otp akan \textit{expire} \\ \hline
            5 & otp\_type & varchar(255) & \textit{field} untuk menyimpan tipe otp \\ \hline
            6 & email & varchar(255) & email yang dikirimkan otp \\ \hline
            7 & is\_active & boolean & apakah otp ini masih aktif atau tidak \\ \hline
        \end{longtable}

        \item \textit{Table contents}\\
        \textit{Primary key}: content\_name \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 4 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{contents}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & content\_name & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{contents} \\ \hline
            4 & content & varchar(255) & isi dari \textit{contents} \\ \hline
        \end{longtable}

        \item \textit{Table activities}\\
        \textit{Primary key}: activity\_id \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 6 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{activities}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & activity\_id & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{activities} \\ \hline
            4 & user\_id & varchar(255) & sebagai \textit{foreign key} untuk tabel \textit{users} \\ \hline
            5 & activity\_value & int & berat dari \textit{activities} tersebut \\ \hline
            6 & activity & varchar(255) & aktifitas \textit{user} dalam bentuk json\\ \hline
        \end{longtable}

        \item \textit{Table knowledge}\\
        \textit{Primary key}: knowledge\_key \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 5 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{knowledge}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & knowledge\_key & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{knowledge} \\ \hline
            4 & knowledge & varchar(255) & sebagai \textit{value knowledge} ini \\ \hline
        \end{longtable}

        \item \textit{Table dictionaries}\\
        \textit{Primary key}: dictionary\_name \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 5 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{dictionaries}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & dictioanry\_name & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{dictionaries} \\ \hline
            4 & dictionary\_type & varchar(255) & tipe dari \textit{dictionary} \\ \hline
            5 & display\_name & varchar(255) & bagaimana \textit{dictionary} ini akan ditampilkan di FE \\ \hline
        \end{longtable}

        \newpage
        \item \textit{Table images}\\
        \textit{Primary key}: image\_id \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 6 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{images}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & image\_id & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{images} \\ \hline
            4 & image\_name & nama dari gambar tersebut \\ \hline
            5 & image\_ext & \textit{extension} dari gambar tersebut \\ \hline
            6 & image & longblob & gambar \\ \hline
        \end{longtable}

        \item \textit{Table reset\_password}\\
        \textit{Primary key}: uuid \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 6 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{reset\_password}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & uuid & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{reset\_password} \\ \hline
            4 & email & varchar(255) & email yang me-\textit{request reset password} \\ \hline
            5 & expired\_time & timestamp & \textit{expired time} untuk \textit{request reset password} \\ \hline
            6 & is\_active & boolean & apakah \textit{request} ini masih aktif atau tidak \\ \hline
        \end{longtable}

        \item \textit{Table email\_encrypt}\\
        \textit{Primary key}: uuid \\
        \textit{Foreign key}: - \\
        Jumlah \textit{field}: 4 \\
        \begin{longtable}{|l|l|l|l|}
            \caption{kamus data untuk tabel \textit{email\_encrypt}}\\
            \hline
            \textbf{No} & \textbf{Nama field} & \textbf{Type} & \textbf{Keterangan} \\ \hline
            \endfirsthead
            %
            ...
            1 & gmt\_create & timestamp & untuk menyimpan waktu create data \\ \hline
            2 & gmt\_modified & timestamp & untuk menyimpan waktu terakhir data diubah \\ \hline
            3 & uuid & varchar(255) & sebagai \textit{primary key} untuk tabel \textit{email\_encrypt} \\ \hline
            4 & email & varchar(255) & email yang me-\textit{request encrypt email} \\ \hline
        \end{longtable}
    \end{itemize}
\end{enumerate}
...
Berikut adalah spesifikasi perangkat yang diperlukan untuk menjalankan aplikasi web Shumishumi baik untuk pengguna.
	
Spesifikasi minimum yang dibutuhkan oleh pengguna aplikasi web Shumishumi adalah minimum spesifikasi yang dibutuhkan untuk menjalankan sebuah \textit{browser} untuk membuka aplikasi web kami, adalah sebagai berikut:

\begin{centering}
\captionof{table}{Tabel spesifikasi}
...
Di halaman aktivasi ini, pengguna mengisi kode yang diterima dari email lalu tekan tombol \textit{Activate} untuk aktivasi akun supaya dapat masuk ke aplikasi Shumishumi. Jika pengguna membutuhkan dikirim email dan kode baru maka ada tombol \textit{Resend} untuk melakukan email dengan kode lagi ke email yang terdaftar dari halaman \textit{register}.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        Untuk aturan emas kedua ini, aplikasi Shumishumi ini memiliki fitur yang cocok yaitu dengan \textit{font} yang mudah dibaca, tampilan warna pada aplikasi yang tidak tabrakan dan \textit{responsive} sehingga bisa diakses dan digunakkan di perangkat yang kecil.

        \item \textit{Offer Informative Feedback}
        
        Aplikasi Shumishumi dianggap sudah memenuhi aturan emas yang ketiga ini, aplikasi Shumishumi akan memberikan ataupun menampilkan informasi kepada penggunanya untuk setiap aksi yang telah dilakukannya. Seperti contoh, saat pengguna memasukan barang ke dalam \textit{wishlist} akan memunculkan notifikasi diatas yang mengabarkan status proses tersebut apakah berhasil atau gagal.

        \begin{figure}[h]
            \centering
            \includegraphics*[scale=0.5]{images/bukti evaluasi ui/Notifikasi Wishlist.png}
            \caption{Contoh notifikasi respons sistem terhadap aksi masukan barang ke wishlist}
        \end{figure}

        \newpage
        \item \textit{Design Dialogs to Yield Closure}
        
        Aplikasi Shumishumi memenuhi aturan emas yang keempat ini, dimana dalam proses aksi ketika sudah selesai ada sebuah respons dari aplikasi yang menandakan dan memberitahu kepada pengguna bahwa proses tersebut sudah selesai. Dapat dicontohkan dengan proses memperbaharui profil yang memiliki respons bahwa profil selesai dan berhasil diperbarui dan juga proses transaksi yang di halaman transaksi akan ada informasi dimana transaksi tersebut telah selesai yang ditandakan dengan status \textit{"Done"}.
        ...
        Aplikasi  Shumishumi ini telah dilengkapi dengan fitur untuk mengembalikan suatu aksi yang telah dilakukan oleh user. Seperti tombol hapus yang ada pada halaman \textit{cart} apabila pengguna tidak jadi untuk membeli produk tersebut. 
        ...
        Aplikasi Shumishumi ini memenuhi aturan ini dengan fiturnya dimana pengguna dengan mudah dapat melakukan perbahuan informasi personal, mengatur \textit{cart} dan \textit{wishlist} sendiri dan juga mencari dan filter barang.

        \item \textit{Reduce Short-Term Memory Load}
        
        Untuk aturan emas yang terakhir ini, aplikasi Shumishumi dianggap telah menguasainya. Seperti peletakan tombol-tombol pada aplikasi yang mudah untuk diingat hingga fitur-fitur yang dapat mengingat data apa saja yang pengguna telah masukkan ke dalam aplikasi, sehingga pengguna tidak perlu lagi memasukkannya kembali satu per satu. Salah satu contohnya adalah fitur \textit{remember me} pada halaman login yang dapat mengingat \textit{session} pengguna sehingga mereka tidak perlu login kembali apabila secara sengaja/tidak sengaja menutup aplikasi web Shumishumi pada \textit{web browser} mereka.

    \end{enumerate}
    \newpage
    \item Evaluasi \textit{User Interface} berdasarkan lima faktor manusia terukur
    
    Untuk mengevaluasi lebih lanjut desain \textit{user interface} pada aplikasi Shumishumi ini, maka dilakukan sebuah pengujian langsung terhadap para penggunanya dengan menggunakan suatu pengukuran tertentu yakni, berdasarkan atas lima faktor manusia terukur (\textit{five measurable human factor}). Cara yang digunakan pada penelitian ini adalah dengan menyebarkan survei kepada para calon pengguna untuk menguji langsung aplikasi tersebut. Para calon pengguna yang telah mengisi survei ada sebanyak 141 responden yang berperan sebagai pembeli biasa dan 35 sebagai penjual (atau pembeli yang telah merubah \textit{role} nya pada aplikasi menjadi penjual). Di dalam survei diberikan tautan yang menuju ke alamat \textit{website} yang sudah di \textit{deploy} dan sebuah video demonstrasi dari aplikasi Shumishumi ini. Melalui tautan yang telah diberikan, para pengguna dapat langsung menguji dan menilai desain dari aplikasi web Shumishumi. Ada empat pertanyaan yang diberikan (dua untuk pembeli dan dua untuk penjual) dan tujuh pernyataan yang perlu dinilai kesetujuannya atau tidak oleh para pengguna. Skala penilaian dari pernyataan-pernyataan tersebut berkisar antara angka \textbf{satu} hingga \textbf{lima}, satu untuk 'sangat tidak setuju' dan lima untuk 'sangat setuju'. Berikut adalah hasil dari survei yang telah diberikan berdasarkan lima faktor manusia terukur yang telah diuji: 

    \begin{itemize}
        \item \textit{Time for users to learn specific functions}
        
        Ada dua pernyataan yang mengaju pada faktor pertama ini, kedua pernyataan yang diajukan adalah, \textit{"User interface dari website shumishumi mudah dipelajari"} dan \textit{"User interface dari website shumishumi enak untuk dilihat"}. Untuk pernyataan pertama sebanyak 70.9\% dari 141 pengguna menjawab sangat setuju ('5'). Sedangkan untuk pernyataan kedua, sebanyak 66.7\% menjawab sangat setuju ('5'). Hal ini menandakan bahwa sebagian besar pengguna setuju bahwa aplikasi web Shumishumi cukup mudah untuk dipelajari dan dilihat.
        ...
        Untuk faktor manusia yang kedua ini, terdapat dua pernyataan juga yang mengajukannya. Pernyataan pertama adalah \textit{"User interface dari website shumishumi mudah untuk dinavigasi"} dan \textit{"Tidak ada proses yang berbelit - belit diwebsite shumishumi"}. Maksud dari kedua pernyataan tersebut adalah untuk menanyakan para penggunanya apakah aplikasi web Shumishumi ini memiliki desain yang susah untuk dinavigasi dan berbelit-belit sehingga para pengguna kesulitan untuk menyelesaikan suatu aksi yang dapat memperlambat alur kerja mereka. Untuk hasilnya sendiri, pada pernyataan pertama untuk faktor terukur kedua ini, sebanyak 63.1\% pengguna menjawab sangat setuju ('5'). Sedangkan untuk pernyataan kedua, sebanyak 63.1\% menjawab sangat setuju ('5'). Itu artinya, masih banyak dari antara pengguna yang setuju bahwa desain aplikasi web Shumishumi tidak memperlambat waktu mereka untuk melakukan berbagai jenis aksi di dalam aplikasi ini. 
...
\appendices{Halaman Originalitas}
\includepdf[page=1]{pdf/Halaman Originalitas.pdf}
