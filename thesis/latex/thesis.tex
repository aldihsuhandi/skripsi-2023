...
\begin{enumerate}
    ...
    UML \textit{Class Diagram} digunakan untuk menampikan kelas-kelas objek yang dipakai pada sebuah sistem. Salah satu jenis \textit{class diagram} yang dipakai untuk menunjukkan hal-hal yang berhubungan dengan domain masalah pengguna disebut dengan \textit{domain model class diagram}, sedangkan jenis UML \textit{class diagram} yang digunakan untuk merancang suatu \textit{software class} pada sistem disebut sebagai \textit{design class diagram}\autocite[103-114]{uml-satzinger}. Berikut adalah penjelasan lebih detail dari kedua jenis \textit{class diagram} tersebut:
    ...
    \begin{enumerate}
        ...
        Dapat dilihat dari gambar diatas (\textit{figure 2.10}) bahwa nama \textit{class} tersebut adalah \textit{Customer}, sedangkan \textit{custNumber, name, phoneNumber, email} dan \textit{billAddress} adalah bentuk dari atribut yang dimiliki oleh \textit{class} tersebut.
        ...
        \begin{itemize}
            ...
            Gambar diatas (\textit{figure 2.12}) merupakan salah satu contoh bentuk dari \textit{domain model class diagram} dengan \textit{multiplicity}-nya. Disini, setiap \textit{class customer} dapat mempunyai 0 (nol) hingga banyak pesanan (\textit{order}) dan setiap pesanan (\textit{order}) dapat memiliki 1 (satu) atau banyak barang yang dipesan (\textit{OrderItem}).  
            ...
            \begin{itemize}
                ...
                Pada gambar diatas (\textit{figure 2.13}), dapat dilihat bahwa \textit{Car, Bus} dan \textit{Truck} merupakan tipe-tipe spesial dari \textit{MotorVehicle}, sedangkan \textit{SportsCar, Sedan} dan \textit{BigCar} adalah tipe-tipe spesial dari \textit{Car}. 
    ...
    \textit{Activity Diagram} merupakan sebuah diagram UML yang digunakan untuk menggambarkan berbagai aktivitas pengguna (atau sistem), seseorang atau komponen yang menyelesaikan setiap aktivitas dan aliran berurutan dari aktivitas-aktiviats tersebut\autocite[60-63]{uml-satzinger}. Berikut adalah rangkuman dari simbol-simbol yang digunakan dalam \textit{activity diagram}:
