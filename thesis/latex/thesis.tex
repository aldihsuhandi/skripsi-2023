...
\subsubbab{2.1.6 \textit{Unified Modeling Languages} (UML)}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{figure}[h]
            \centering
            \includegraphics[width=4cm]{class example from class diagram.png}
            \caption{Contoh dari \textit{class}}
        \end{figure}
        ...
        \textit{Operations} atau juga bisa disebut sebagai \textit{method}/(metode), ialah sebuah aksi atau fungsi (\textit{functions}) yang dapat dilakukan oleh \textit{class} tersebut. \textit{Operations} dibagi ke dalam empat jenis yakni, \textit{constructor, query, update} dan \textit{destructor}.
        \begin{itemize}
            \item \textit{Constructor Operation}\\
            \textit{Constructor Operation} merupakan sebuah operasi yang bertugas untuk membuat (\textit{create}) sebuah \textit{instance} dari \textit{class}. Biasanya, \textit{operation} jenis ini tidak secara eksplisit ditampilkan di dalam \textit{class diagram} dikarenakan hanya mengimplementasikan sebuah \textit{function} yang \textit{basic}. Contoh: sebuah \textit{class} bernama "Pasien" memiliki sebuah \textit{method} bernama \textbf{insert()}, yang dimana \textit{method} tersebut akan membuat sebuah \textit{instance} pasien baru ketika ada pasien yang masuk ke dalam sistem tersebut\autocite{systemanalysisdesign-class-diagram}.
            \item \textit{Query Operation}\\
            \textit{Query Operation} WIP
